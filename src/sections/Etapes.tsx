...
export function Etapes() {
  ...
  return (
    <Section id={stages.id} title={stages.title} eyebrow="01 · Terraformation">
      ...
      <p className="mb-8 rounded-md border border-amber/40 bg-amber/10 px-3 py-3 text-sm text-cream sm:px-4">
        {stages.variance}
      </p>
      <TIStageSlider />
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {stages.stages.map((s, i) => (
          <button
            ...
            className={cn(
              'min-w-0 rounded-md border p-3.5 text-left transition sm:p-4',
              i === stageIndex
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <span className="min-w-0 break-words font-headline font-semibold text-cream">
                {s.name}
              </span>
              <span className="shrink-0 break-all text-right font-mono text-[10px] text-cream/50 sm:text-xs">
                {s.threshold}
              </span>
            </div>
            <p className="mt-2 line-clamp-3 text-xs text-cream/65">{s.description}</p>
            {'thresholdNote' in s && s.thresholdNote ? (
              <p className="mt-2 break-words text-[10px] text-amber">{s.thresholdNote}</p>
            ) : null}
          </button>
        ))}
      </div>

      <GlassPanel className="mt-8 min-w-0 overflow-hidden" as="article">
        <h3 className="font-headline break-words text-lg font-semibold">
        ...
        <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {stagesSelenea.stages.map((s) => (
            <div
              key={s.id}
              className="min-w-0 rounded-md border border-cream/10 bg-cream/5 p-3"
            >
              <div className="flex items-start justify-between gap-2">
                <span className="min-w-0 break-words text-sm font-medium">{s.name}</span>
                <span className="shrink-0 break-all text-right font-mono text-[10px] text-cream/50">
                  {s.threshold}
                </span>
              </div>
