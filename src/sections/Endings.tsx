...
export function Endings() {
  return (
    <Section id={endings.id} title={endings.title} eyebrow="04 · Fins">
      <p className="mb-6 max-w-3xl">{endings.intro}</p>
      <div
        className="mb-8 rounded-2xl border border-coral/40 bg-coral/10 px-4 py-4 text-sm text-cream"
        role="alert"
      >
        {endings.warning}
      </div>
      <div className="grid gap-4 lg:grid-cols-3">
        {endings.routes.map((r) => (
          <GlassPanel key={r.id} as="article">
            ...
            <p className="mt-1 text-xs uppercase tracking-wider text-teal">{r.keys}</p>
            <p className="mt-3 text-sm">{r.summary}</p>
            <ol className="mt-3 list-decimal space-y-1 pl-5 text-xs text-cream/65">
              {r.steps.map((s) => (
                <li key={s}>{s}</li>
              ))}
            </ol>
      ...
      <GlassPanel className="mt-6">
        ...
        <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-cream/70">
