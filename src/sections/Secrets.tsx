...
export function Secrets() {
  return (
    <Section id={secrets.id} title={secrets.title} eyebrow={secrets.badge}>
      <p className="mb-8 max-w-3xl">{secrets.intro}</p>
      <div className="grid gap-4 md:grid-cols-2">
        {secrets.items.map((item) => (
          <GlassPanel key={item.title} as="article">
            <div className="mb-2 inline-flex rounded-full border border-white/15 px-2 py-0.5 text-[10px] uppercase tracking-wider text-white/55">
              {item.source}
            </div>
