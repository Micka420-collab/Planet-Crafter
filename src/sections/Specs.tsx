...
export function Specs() {
  return (
    <Section id={specs.id} title={specs.title} eyebrow="Dossier · Specs">
      <div className="grid gap-4 lg:grid-cols-3">
        <GlassPanel as="article">
          <h3 className="font-headline text-lg font-semibold">PC — minimum</h3>
          <p className="mt-1 text-xs text-cream/45">{specs.pcSpecs.note}</p>
          <ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-cream/75">
        ...
        <GlassPanel as="article">
          ...
          <ul className="mt-3 flex flex-wrap gap-2">
            {specs.achievements.categories.map((c) => (
              <li key={c} className="rounded-full border border-cream/15 px-2.5 py-1 text-xs">
                {c}
              </li>
            ))}
          </ul>
